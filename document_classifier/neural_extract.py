...
EXTRACT_PREFIX = (
    ...
    "«Поставщик»: одна связная строка — наименование юрлица/ИП (ООО/АО/ПАО/ИП и т.п.) рядом с ИНН поставщика; "
    "при отсутствии слова «Поставщик» используй «Продавец»/«Исполнитель»/«Получатель»/«Покупатель»/«Заказчик», где рядом ИНН поставщика. "
    "В «Поставщик» не включай ИНН/ОГРН/ОГРНИП/р/с/к/с и любые длинные числа (10+ цифр подряд) — они не часть названия; "
    "ИНН выноси только в «ИНН поставщика». "
    "Не добавляй субъекты РФ («… область», «… край», «Республика …»), не дублируй регион после ФИО и не склеивай обрывки OCR "
    "(перенос строки внутри «… область», искажённые повторы частей фамилии). "
    "Если в тексте название смешано с цифрами и мусором — восстанови только читаемое наименование или поставь null. "
    "Не подставляй адрес, телефон, банк, БИК, юридические фразы («обязуется поставить», оферта) и одиночные адресные слова. "
