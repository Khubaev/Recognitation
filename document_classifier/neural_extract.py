...
EXTRACT_PREFIX = (
    "Извлеки из документа один JSON-объект и верни только его, без комментариев и Markdown. "
    "Строгая схема JSON: "
    "{\"Номер счета\":\"\",\"Дата счета\":\"\",\"ИНН поставщика\":\"\",\"Поставщик\":\"\",\"Итого\":\"\"}. "
    "Правила: "
    "1) «Номер счета» только из строки «Счет/Счёт ... № ...»; не путай с банковским «Сч. №», «р/с», «к/с» (длинное число 18-22 цифры). "
    "2) «Дата счета» бери из той же строки с номером (обычно «от ...»), формат строго ДД.ММ.ГГГГ. "
    "3) «ИНН поставщика» бери из блока поставщика/продавца/исполнителя. "
    "4) «Поставщик» — только наименование юрлица/ИП (ООО/АО/ПАО/ИП и т.п.) рядом с ИНН поставщика. "
    "Если явного слова «Поставщик» нет, используй блоки «Продавец»/«Исполнитель»/«Получатель»/«Покупатель»/«Заказчик», где рядом ИНН поставщика. "
    "Не используй адрес, телефон, банк, БИК, ИНН/КПП, юридические фразы («обязуется поставить», «в соответствии с настоящим...») "
    "и одиночные адресные слова («БРАТИСЛАВСКАЯ», «ЛЕНИНСКИЙ»). "
    "5) «Итого» — только сумма цифрами. Если есть «Итого с НДС»/«Всего с НДС», бери ее в приоритете; "
    "иначе «Итого к оплате»/«Всего к оплате»/«К оплате». Не возвращай сумму прописью. "
    "6) Если значение не найдено — пустая строка. Другие ключи не добавляй. "
    "Текст документа:\n"
)
